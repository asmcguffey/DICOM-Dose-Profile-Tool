...
class ProfileTool:
    ...
    def _get_datasets_from_folder(self):
        ...
        if dict_of_datasets:
            ...
            if all(
                [(modality in dict_of_datasets) for modality in required_modalities]
            ):
    ...
    def get_profile(
        ...
        spacing=0.1,
        return_coords=True,
        profile_dose_units="Gy",
        normalize_by_monitor_units=False,
        zero_point="default",
    ):
        ...
        if isinstance(self._dose_reader, RTDoseReader):
            ...
            if self._dose_reader.dose_units == "GY":
                ...
                raise ValueError(
                    'DICOM dose units == "RELATIVE". Relative dose not supported.'
                )
            ...
            if normalize_by_monitor_units:
                if isinstance(self._plan_reader, RTPlanReader):
                    if "Monitor Units" in self._plan_reader.beam_parameters:
                        monitor_units = float(
                            self._plan_reader.beam_parameters["Monitor Units"]
                        )
    ...
    def get_zero_point(self, mode="INTERCEPT_CAX_SURFACE") -> list:
        ...
        if mode == "INTERCEPT_CAX_SURFACE":
            ...
            if not all(
                [
                    key in beam_params_dict
                    for key in ["Isocenter", "Gantry Angle", "SSD"]
                ]
            ):
                return None
            else:
        ...
        if isinstance(self._dose_reader, RTDoseReader):
            ...
            with open(fp, "r") as csv_file:
                for i, line in enumerate(csv_file.readlines()):
                    if i > skip_rows - 1:
                        points = [
                            float(v) for v in line.strip().split(delimiter) if v != ""
                        ]
