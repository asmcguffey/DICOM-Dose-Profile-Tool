...
def popup_view_dicom_info(dict_of_datasets):
    ...
    tabs = sg.TabGroup(
        [[sg.Tab(
            key, [[sg.Multiline(str(dataset), **multiline_kw)]]
...
def display():
    ...
    config_frame_layout = [
        [
            ...
            sg.Push(),
            sg.Input(key='-DICOM FOLDER PATH-', tooltip='Path to DICOM folder. Must contain \nexactly 1 RTDOSE and 1 RTPLAN file.'),
            sg.FolderBrowse('Browse...')],

        [
            sg.Text('Points File (.csv)'),
            sg.Push(), sg.Input(key='-POINTS FILE PATH-', tooltip='Path to csv file defining profiles.'),
    ...
    layout = [
        ...
        [
            ...
            sg.FileSaveAs('Write Profiles', target='-SAVE AS-', file_types=(('CSV Files', '.csv'),)),
            sg.Cancel(),
            sg.Push(),
    ...
    while True:             
        event, values = window.read()
        if event == '-SAVE AS-':
            if all([values[key] for key in ['-DICOM FOLDER PATH-', '-POINTS FILE PATH-', '-SAVE AS-']]):
                pt = ProfileTool(
                    folder=values['-DICOM FOLDER PATH-'],
                    interpolation_method=interpolation_options[values['-INTERPOLATION METHOD-']])
